...
\newline
\newline
The next step is to run \texttt{make menuconfig}. This will load a graphical dialog (see Figure 1). With this dialog, you can add or remove modules from the config.
\newline
\newline
Use the up/down arrow keys to navigate vertically. Return is used to select a submenu in the dialog. Typing \texttt{Y} will include a feature. Typing \texttt{N} will exclude it. Use the left/right arrow keys to navigate to \texttt{Exit} when you want exit a submenu or the dialog. 
